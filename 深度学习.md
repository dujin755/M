...
![图片](https://zh.d2l.ai/_images/gru-1.svg)  
  
##### 8.1.1.2 候选隐单位  
图9.1.2说明了应用重置门之后的计算流程  
![图片](https://zh.d2l.ai/_images/gru-2.svg)  
##### 8.1.1.3 隐状态  
图9.1.3说明了更新门起作用后的计算流  
![图片](https://zh.d2l.ai/_images/gru-3.svg)  
...
```  
##### 8.1.2.1 初始化模型参数  
标准差为 0.01 的高斯分布中提取权重，并将偏置项设为 0，超参数num_hiddens定义隐藏单元的数量， 实例化与更新门、重置门、候选隐状态和输出层相关的所有权重和偏置  
```python 
...
> 为了控制记忆元，需要许多门  
其中一个门用来从单元中输出条目，将其称为输出门  
另外一个门用来决定何时将数据读入单元，将其称为输入门  
还需要一种机制来重置单元的内容，由遗忘门来管理， 这种设计的动机与门控循环单元相同， 能够通过专用机制决定什么时候记忆或忽略隐状态中的输入  

##### 8.2.1.1 输入门、忘记门和输出门  
就如在门控循环单元中一样， 当前时间步的输入和前一个时间步的隐状态作为数据送入长短期记忆网络的门中  
![](https://zh.d2l.ai/_images/lstm-0.svg)  
由于三个门都运用了sigmoid 函数，所以它们三个值都在(0,1)之间  

##### 8.2.1.2 候选记忆元  
计算与上面描述的三个门的计算类似，但是使用 tanh 函数作为激活函数，函数的值范围为 (-1,1)  
![](https://zh.d2l.ai/_images/lstm-1.svg)  
##### 8.2.1.3 记忆元  
在门控循环单元中，有一种机制来控制输入和遗忘（或跳过）   
在长短期记忆网络中，也有两个门用于这样的目的： 输入门控制采用多少来自
的新数据， 而遗忘门控制保留多少过去的 记忆元的内容  

![](https://zh.d2l.ai/_images/lstm-2.svg)  
##### 8.2.1.4 隐状态  
只要输出门接近 1，就能够有效地将所有记忆信息传递给预测部分  
而对于输出门接近 0，只保留记忆元内的所有信息，而不需要更新隐状态  
...
![](https://zh.d2l.ai/_images/deep-rnn.svg)  
...
因为有不同的词元，所以输入和输出都选择相同数量，即vocab_size  
现在可以通过num_layers的值来设定隐藏层数  
...
#### 8.4.1 隐马尔可夫模型的动态规划  
#### 8.4.2 双向模型  
增加一个“从最后一个词元开始从后向前运行”的循环神经网络， 而不是只有一个在前向模式下“从第一个词元开始运行”的循环神经网络  
 双向循环神经网络添加了反向传递信息的隐藏层，以便更灵活地处理此类信息  
![](https://zh.d2l.ai/_images/birnn.svg)  
...
因为机器翻译正是将输入序列转换成输出序列的 序列转换模型的核心问题  
...
第一个组件是一个编码器 ： 它接受一个长度可变的序列作为输入， 并将其转换为具有固定形状的编码状态  
第二个组件是解码器 ：它将固定形状的编码状态映射到长度可变的序列  

#### 8.6.1 编码器  
在编码器接口中，只指定长度可变的序列作为编码器的输入X  
```python
from torch import nn
...
为了逐个地生成长度可变的词元序列， 解码器在每个时间步都会将输入和编码后的状态 映射成当前时间步的输出词元  
...
可以使用下面的sequence_mask函数 通过零值化屏蔽不相关的项， 以便后面任何不相关预测的计算都是与零的乘积，结果都等于零  
```python
#@save
...
现在，可以通过扩展softmax交叉熵损失函数来遮蔽不相关的预测  
最初，所有预测词元的掩码都设置为1  
...
### 9.1 注意力提示  
#### 9.1.1 查询、键与值  
在注意力机制的背景下，自主性提示被称为查询  
给定任何查询，注意力机制通过注意力汇聚将选择引导至感官输入  

在注意力机制中，这些感官输入被称为值  
更通俗的解释，每个值都与一个键配对， 这可以想象为感官输入的非自主提示  

![](https://zh.d2l.ai/_images/qkv.svg)  
...
```  
#### 9.2.1 生成数据集  
根据下面的非线性函数生成一个人工数据集， 其中加入的噪声项为<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mi>&#x3F5;</mi>
</math> ：  
...
如图 ：  
![](https://zh.d2l.ai/_images/output_nadaraya-waston_736177_39_0.svg)   
...
**K** 是核  
该公式所描述的估计器被称为 Nadaraya-Watson 核回归  
...
![](https://zh.d2l.ai/_images/transformer.svg)  
从宏观角度来看，Transformer的编码器是由多个相同的层叠加而成的，每个层都有两个子层  
第一个子层是多头自注意力汇聚；第二个子层是基于位置的前馈网络  

在计算编码器的自注意力时，查询、键和值都来自前一个编码器层的输出，且每个子层都运用了残差连接  
除了编码器中描述的两个子层之外，解码器还在这两个子层之间插入了第三个子层，称为编码器－解码器注意力层  

在解码器自注意力中，查询、键和值都来自上一个解码器层的输出  
#### 9.4.2 基于位置的前端网络  
基于位置的前馈网络对序列中的所有位置的表示进行变换时使用的是同一个多层感知机，这就是称前馈网络是基于位置的原因  
```python
#@save
...
为了在预测过程中得到确切的结果，通常对训练样本只进行图像增广，且在预测过程中不使用随机操作的图像增广  

接下来，定义一个辅助函数，以便于读取图像和应用图像增广  
```python
...
* 在数据集上预测训练神经网络模型，即源模型   
* 创建一个新的神经网络模型，即目标模型  
这将复制源模型上的所有模型设计及其参数  
* 向目标模型添加输出层，其输出数是目标数据集中的类别数  
然后随机初始化该层的模型参数  
* 在目标数据集（如椅子数据集）上训练目标模型  
输出层将从头开始进行训练，而所有其他层的参数将根据源模型的参数进行微调  

具体流程如下 ：  
![](https://zh.d2l.ai/_images/finetune.svg)  
...
```  
##### 10.2.2.1 获取数据集  
解压下载的数据集，获得了两个文件夹hotdog/train和hotdog/test  
```python
#@save
...
在训练期间，首先从图像中裁切随机大小和随机长宽比的区域，然后将该区域缩放为<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mn>224</mn>
  <mo>&#xD7;</mo>
  <mn>224</mn>
</math>输入图像  

在测试过程中，将图像的高度和宽度都缩放到256像素，然后裁剪中央<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mn>224</mn>
  <mo>&#xD7;</mo>
  <mn>224</mn>
</math>区域作为输入  
...
### 10.3 目标检测和边界框  
很多时候图像里有多个感兴趣的目标，不仅想知道它们的类别，还想得到它们在图像中的具体位置  
在计算机视觉里，我们将这类任务称为目标检测或目标识别  
...
### 10.4 区域卷积神经网络系列  
R-CNN首先从输入图像中选取若干（例如2000个）提议区域（如锚框也是一种选取方法），并标注它们的类别和边界框  
...
#### 10.4.1 Fast R-CNN  
![](https://zh.d2l.ai/_images/fast-rcnn.svg)  

它的主要计算如下：  

* 与R-CNN相比，Fast R-CNN用来提取特征的卷积神经网络的输入是整个图像，而不是各个提议区域  
此外，这个网络通常会参与训练  
设输入形状为 <math xmlns="http://www.w3.org/1998/Math/MathML">
  <mn>1</mn>
  <mo>&#xD7;</mo>
  <mi>c</mi>
  <mo>&#xD7;</mo>
...
* 然后，这些感兴趣的区域需要进一步抽取出形状相同的特征，以便于连结后输出  
为了实现这一目标，Fast R-CNN引入了兴趣区域汇聚层 ：将卷积神经网络的输出和提议区域作为输入，输出连结后的各个提议区域抽取的特征，形状为<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mi>n</mi>
  <mo>&#xD7;</mo>
  <mi>c</mi>
  <mo>&#xD7;</mo>
...
* 通过全连接层将输出形状变换为<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mi>n</mi>
  <mo>&#xD7;</mo>
  <mi>d</mi>
</math>，其中超参数d取决于模型设计  
...
#### 10.4.2  Faster R-CNN  
![](https://zh.d2l.ai/_images/faster-rcnn.svg)  
计算步骤如下：  

* 使用填充为1的<math xmlns="http://www.w3.org/1998/Math/MathML">
  <mn>3</mn>
  <mo>&#xD7;</mo>
  <mn>3</mn>
</math>的卷积层变换卷积神经网络的输出，并将输出通道数记为c  
* 以特征图的每个像素为中心，生成多个不同大小和宽高比的锚框并标注它们  
* 使用锚框中心单元长度为c的特征，分别预测该锚框的二元类别和边界框  
* 使用非极大值抑制，从预测类别为目标的预测边界框中移除相似的结果  
最终输出的预测边界框即是兴趣区域汇聚层所需的提议区域  


#### 10.4.3 Mask R-CNN  
![](https://zh.d2l.ai/_images/mask-rcnn.svg)  

### 10.5 语义分割和数据集  
本节将探讨语义分割问题，它重点关注于如何将图像分割成属于不同语义类别的区域   
#### 10.5.1 图像分割与实例分割  

* **图像分割** ： 将图像划分为若干组成区域，这类问题的方法通常利用图像中像素之间的相关性  
在训练时不需要有关图像像素的标签信息，在预测时也无法保证分割出的区域具有我们希望得到的语义  
* **实例分割** ： 也叫同时检测并分割，它研究如何识别图像中各个目标实例的像素级区域  
